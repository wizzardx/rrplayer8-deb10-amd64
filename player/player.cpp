...
void player::init() {
  ...
  log_message("Today's RR date: " + datetime_to_rrdate(date()));
  ...
  log_message("Connecting to instore database (" + config.db.strdb + " on " + config.db.strserver + ")...");
...
void player::get_next_item_promo(programming_element & item, const int intstarts_ms) {
  ...
  while (!RS.eof() && AnnounceList.size() < (unsigned) config.intmax_promos_per_batch) {
    ...
    // v6.14.3 - PAYB stuff:
    testing; blnCheckPrerecLifespan = false;
  ...
  if (AnnounceList.size() > 0) {
    ...
    // XMMS, and when an ad finished, it wotesting;uld check for the next ad, or resume music playback.
...
testing_throw;
            ...
            testing; // Check if we are at the same position in the queue as before!
          }
        }
