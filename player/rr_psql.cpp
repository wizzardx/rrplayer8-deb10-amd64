...
/********************************************************************************************
          A Connection wrapper - logs errors, does NOT throw exceptions,
          except when running queries.
********************************************************************************************/
...
pg_connection::pg_connection() {
  ...
  pconn = NULL;
  ptransaction = NULL;
  intmax_retries = PG_RETRY_INFINITE;
...
void pg_connection::open(const string & strConn) {
  ...
  if (pconn != NULL) {
    rr_throw("Connection is already open! Call close() first.");
  }

  // Check: strconn must not be empty.
  if (strconn == "") {
    rr_throw("Connection string is empty.");
  }
...
void pg_connection::check() {
  // Check: pg_conn must not be NULL.
  if (pconn == NULL) {
    rr_throw("Connection is not meant to be open, check() should not be called. Call open() first.");
  }
  establish_connection();
}

// Is the connection open? This method does not actively check the connection or attempt to reconnect.
bool pg_connection::ok() {
  bool blnret = false;
  if (pconn != NULL) {
...
pg_result pg_connection::exec(const string & strsql) { // An exception is thrown if there is a SQL execution error
  ...
  if (ptransaction == NULL) rr_throw("Transaction object not initialized");
  ...
  try {
    ...
    if (StringToUpper(substr(strError, 0, 5)) == "FATAL") {
    ...
    rr_throw("SQL query failed. The query: \"" + strsql + "\". The error: \"" + strError + "\"");
  }
}

void pg_connection::call_event_on_conn_error(event & Event) {
  // Set a pointer to code (An "event") to be called if a connection error occurs...
  client_conn_err_code = &Event;
}
...
void pg_connection::establish_connection() {
  ...
  if (strconn == "") {
    rr_throw("Connection string not set!");
  }
  ...
  while (!blnloop_done) {
    ...
    if (blnConnected) {
      ...
      if (client_conn_err_code != NULL) {
        log_message("Connection error occured, running custom client code...");
        client_conn_err_code->run(); // in the case of the player, this assures that music playback does take place.
      }
      ...
      if ((lngattempt_num <= intmax_retries) || (intmax_retries == PG_RETRY_INFINITE)) {
        ...
        blnloop_done = true;        
        rr_throw("Could not establish a connection to the database.");
      } // end else
    }
  }
}
...
/********************************************************************************************************
          A Result wrapper - logs errors and rethrows exceptions
*********************************************************************************************************/
// Copy constructor
  ...
  try {
    ...
    if ((field.is_null()) || (field.c_str() == NULL)) {
      if (strdefault_val == NULL) {
        ...
        rr_throw("Field \"" + strfield_name + "\" is NULL (and you did not specify a \"default\" value). Please check the data! Record #" + itostr(introw_num) + ", returned from query: " + strsql);
      }
...
// Resultset traversal:
bool pg_result::eof() {
  // Return true if the current result position is at or after the eof of the record.
  return (unsigned)introw_num > (*presult).size();
}

void pg_result::movenext() { // An exception is thrown if an attempt is made to move past the EOF.
  if (!eof()) {
    ++introw_num;
  }
  else {
    // We can't move to the next record after moving to the EOF...
    rr_throw("Attempt to move past the EOF");
  }
}

long pg_result::recordcount() {
  return (*presult).size();
}
...
void pg_transaction::commit() {
  if (connection.ptransaction == NULL) rr_throw("Transaction is not setup!");
...
string DateTimeToPSQL(const DateTime dtmDateTime) {
  string strDateTime = format_datetime(dtmDateTime, "%dd/%mm/%yyyy %HH:%nn:%ss");
  string strSQL = "to_timestamp('" + strDateTime + "', 'dd/mm/yyyy hh24:mi:ss')";
  return strSQL;
}

string DateToPSQL(const DateTime dtmDate) {
  string strDate = format_datetime(dtmDate, "%dd/%mm/%yyyy");
  string strSQL = "to_timestamp('" + strDate + "', 'dd/mm/yyyy')";
  return strSQL;
}
...
string GetStringComparePSQL(const string & strString) {
  ...
  if (strString=="")
    ...
    strCompare = "= " + StringToPSQL(strString);
  }
  return strCompare;
}

DateTime ParsePSQLDateTime(const string & strPSQLDate, const bool blnextract_date=false, const bool blnextract_time=false) {
  //
  ...
  const char * DateStr = strPSQLDate.c_str();

  int intdatelen = strlen(DateStr);

  if (intdatelen < 10) {
    rr_throw((string)"date/time string \"" + DateStr + "\" is too short!");
  }
  ...
  // Day
      isdigit(DateStr[8]) &&
      isdigit(DateStr[9]))) {
    rr_throw("Invalid postgresql date/time string \"" + strPSQLDate + "\"");
  }

  // We have the date portion -> extract it
  intYear = atoi(strPSQLDate.substr(0,4).c_str());
  intMonth = atoi(strPSQLDate.substr(5,2).c_str());
  intDay = atoi(strPSQLDate.substr(8,2).c_str());
  ...
  if (intdatelen > 10) {    
    ...
    // Dividing " " char
    if (DateStr[10]!=' ') {
      rr_throw("Char 10 must be a space!");
    }
    ...
    if ((DateStr[11]=='B') && (DateStr[12])=='C') {
      ...
      intHour = atoi(strPSQLDate.substr(11,2).c_str());
      intMinute = atoi(strPSQLDate.substr(14,2).c_str());
      ...
      if (DateStr[16]==':' && isdigit(DateStr[17]) && isdigit(DateStr[18])) {
        intSecond = atoi(strPSQLDate.substr(17,2).c_str());
      } // end if
    } // END: if ((DateStr[11]=='B') && (DateStr[12])=='C')
  ...
  if (blnextract_date) {
    if (intYear < 1970) {
      rr_throw("Year " + itostr(intYear) + " is invalid - I cannot process years before 1970");
    }
  }

  if (blnextract_date && blnextract_time) {
    return_val = MakeDateTime(intYear, intMonth, intDay, intHour, intMinute, intSecond);
  }
  else if (blnextract_date && !blnextract_time) {
    return_val = MakeDate(intYear, intMonth, intDay);
  }
  else if (!blnextract_date && blnextract_time) {
    return_val = MakeTime(intHour, intMinute, intSecond);
  }
  else { // The logic below should never run!
    rr_throw("logic error!");
  }      
...
DateTime parse_psql_datetime(const string & str) {
  return ParsePSQLDateTime(str, true, true);
}

DateTime parse_psql_date(const string & str) {
  return ParsePSQLDateTime(str, true, false);
}

DateTime parse_psql_time(const string & str) {
  return ParsePSQLDateTime(str, false, true);
}
...
string TimeToPSQL(const DateTime dtmTime) {
  string strSQL = "";
  string strTime = format_datetime(dtmTime, "%HH:%nn:%ss");
