...
#include <glib.h> // If you get a "No such file or directory" error, then install deb package "libglib1.2-dev"
#include <sys/types.h>
#include <sys/stat.h>
...
#include <stdlib.h>
#include "xmmsctrl.h"
#include "xmms_controlsocket.h"
#include "rr_utils.h" // For rr_throw

#include "check_library_versions.h" // Always last: Check that the included RR library versions are correct.

using namespace rr; // Access the rr namespace with the rr_utils stuff in it.

const int XMMS_API_INPUT_TIMEOUT=5; // How many seconds to wait for replies from the XMMS API
                                    // before it is determined that XMMS is in fact frozen.

#include <unistd.h>
// Also a macro define, straight from UNISTD. I'm having problems defining the __USE_GNU macro correctly.
...
// Internal input_timeout() function added - this is so that frozen XMMS instances
// do not freeze this API!
...
void kill_xmms() { // New function
  log_warning("Killing XMMS...");  
...
static gpointer remote_read_packet(gint fd, ServerPktHeader * pkt_hdr) {
  ...
  if (input_timeout(fd, XMMS_API_INPUT_TIMEOUT) != 1) {
    kill_xmms(); // Kill XMMS
    rr_throw("XMMS API freeze detected! XMMS killed");
  }
  
  if (!read(fd, pkt_hdr, sizeof (ServerPktHeader)) == sizeof (ServerPktHeader)) {
    rr_throw("Bad server packet header size!");
  }

  if (pkt_hdr->data_length) {
    data = g_malloc0(pkt_hdr->data_length);
    // Check again for input timeouts!
    if (input_timeout(fd, XMMS_API_INPUT_TIMEOUT) !=1) {
      kill_xmms(); // Kill XMMS
      rr_throw("XMMS API freeze detected! XMMS killed");
    }
...
static void remote_send_guint32(gint session, guint32 cmd, guint32 val)
{
	...
	fd = xmms_connect_to_session(session); // Throws an exception if the connection fails
...
gint xmms_connect_to_session(gint session)
{
  ...
  if (connect(fd, (struct sockaddr *) &saddr, sizeof (saddr)) == -1) {
    close(fd);
    rr_throw("Unable to connect to XMMS!");
  }
  return fd; // Success
}
...
void xmms_remote_playlist(gint session, gchar ** list, gint num, gboolean enqueue)
{
   ...
   fd = xmms_connect_to_session(session); // Throws an exception if the connection fails
	...
	{
		data_length += 4;
		data = (gchar *) g_malloc(data_length);
...
void xmms_remote_playlist_add(gint session, GList * list)
{
	...
	str_list = (gchar **)g_malloc0(num * sizeof (gchar *));
	for (i = 0, node = list; i < num && node; i++, node = g_list_next(node))
		str_list[i] = (gchar *)node->data;
...
void xmms_remote_get_volume(gint session, gint * vl, gint * vr)
{
	...
	gint fd;
	gpointer data;
  // Also initialize the return variables... (in case we are don't get a connection)
  *vl=*vr=-1;
