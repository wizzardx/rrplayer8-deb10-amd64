...
void timed_events::add_event(event & Event, const long lngSecondInterval, bool blnEnabled) {
  ...
  if (event_listed(Event)) {
    rr_throw("Cannot add an event more than once!");    
  }

  if (lngSecondInterval <= 0) {
    // Cannot set a negative or 0 interval!
    rr_throw("Cannot add an event with a non-positive interval! (" + itostr(lngSecondInterval) + ")");
  }
...
void timed_events::remove_event(const event & Event) {
  ...
  if (I == EventTiming.end()) {
    rr_throw("Event object not found! Cannot remove it!");    
  }
...
void timed_events::enable_event(const event & Event) {
  ...
  if (I != EventTiming.end()) {
    ...
    // Event not found!
    rr_throw("Event not listed, it cannot be enabled!");
  }
}
...
void timed_events::disable_event(const event & Event) {
  ...
  if (I != EventTiming.end()) {
    ...
    // Event not found!
    rr_throw("Event not listed, it cannot be disabled!");
  }
}
...
void timed_events::set_event_interval(const event & Event, const long lngSecondInterval) {
  ...
  if (I != EventTiming.end()) {
    ...
    if (lngSecondInterval > 0) {    
      ...
      rr_throw("Cannot set an event's interval to a non-positive number of seconds! (" + itostr(lngSecondInterval)+ ")");
    }
  } // end if
  else {
    // Event not found!
    rr_throw("Event not listed, it's interval cannot be set! (to " + itostr(lngSecondInterval) + ")");
  }
}
      ...
      if (I->blnEnabled) {
        ...
        if (dtmnow >= I-> dtmNextRun) {
          ...
          if (I->Event != NULL) {
            ...
            rr_throw("Event object is set to NULL!");
          }
        }
      }
