/***************************************************************************
                          char_array_maths.cpp  -  description
                             -------------------
    version              : v0.03
...
char_array_maths::char_array_maths(unsigned char * buffer, const int intbuff_len) {
  ...
  // Check the arguments...
  if (buffer == NULL) {
    rr_throw("Buffer pointer is null.");
  }
  else if (intbuff_len <= 0) {
    rr_throw("Buffer must be at least 1 byte long (" + itostr(intbuff_len) + " is invalid)");
  }
  else {
    Buffer = buffer;
...
// maths operations. True returned if no problem, False means a problem occured,
// get_problem can be called to fetch a description of the problem.
...
void char_array_maths::multiply(const long long lngmultiply_by) {
  ...
  if (lngmultiply_by < 0) {
    ...
    rr_throw("Multiplication by negative numbers not allowed. Multiplier is " + ltostr(lngmultiply_by));
  }
...
// Check for division by 0
  if (lngdivide_by == 0) {
    // Division by zero
    rr_throw("Division by zero.");
  }
  // Check for division by negative numbers
  else if (lngdivide_by < 0) { 
    // Number to divide by is negative
    rr_throw((string) "Division by a negative number is not allowed. Number to divide by is " + ltostr(lngdivide_by));
  }
  else {
...
void char_array_maths::add(long long lngadd, int intplace) {
  ...
  if (lngadd < 0) {
    rr_throw((string) "Addition of negative numbers not allowed. Number to add is " + ltostr(lngadd));
  }
  // Check that intplace is valid...
  else if (intplace >= intBuffLen) {
    // Place to start adding, is after the end of the buffer!
    rr_throw("Cannot add to buffer position " + itostr(intplace) + ", buffer is only " + itostr(intBuffLen) + " characters long");
  }
  else if (intplace <  intReservedStartChars) {
    // Place is in a reserved area...
    rr_throw("Cannot add to buffer position " + itostr(intplace) + " because the first " + itostr(intReservedStartChars) + " character(s) have been reserved");
  }
  else{
    // Place is valid.
    ...
    if (intPos < intReservedStartChars && lngVal > 0) {
      // An overflow occured
      rr_throw("Overflow occured during addition. Use a larger char buffer.");
    }
  }
}
...
void char_array_maths::set_reserved_start_chars(const int intreserved_start_chars) {
  ...
  if ((intreserved_start_chars >= 0) && (intreserved_start_chars <= intBuffLen)) {
    ...
    rr_throw("Reserved start characters value " + itostr(intreserved_start_chars) + " is not in the allowed range 0 to " + itostr(intBuffLen));
  }
}
...
void char_array_maths::include_value(const long lngvalue, const long lngmin, const long lngmax) {
  ...
  if (lngmin > lngmax) {
    ...
    rr_throw("Invalid range " + ltostr(lngmin) + " to " + ltostr(lngmax) + " (MIN must be <= MAX)");    
  }
  ...
  if ((lngmin <= lngvalue) && (lngvalue <= lngmax)) {
    ...
    rr_throw("Value to include (" + ltostr(lngvalue)+ ") was outside of the specified range " + ltostr(lngmin) + " to " + ltostr(lngmax));
  }
}
...
void char_array_maths::extract_value(long & lngvalue, const long lngmin, const long lngmax) {
  ...
  if (lngmin <= lngmax) {
    ...
    if ((lngResult >= lngmin) && (lngResult <= lngmax)) {
      ...
      rr_throw("Extracted value was out of range!");
    }
  }
  else {
    // lngmin is not < lngmax
...
void char_array_maths::include_string(const string & strString, const string strAllowedChars, const char_repetition char_repeat) {
  // Check the string's length.  
  if (strString.length() == 0) {
    rr_throw("Code is empty.");
  }
  else {
    ...
    if (char_repeat == char_repeat_not_allowed) {
      --intRadix;
    }

    // Is "Radix" a value >= 2?
    if (intRadix < 2) {
      rr_throw("Invalid Radix (numeric base) " + itostr(intRadix) + ". Check the allowed characters string is \"" + strAllowedChars + "\"");
    }

    int intcode_pos = strString.length() - 1;
    while ((intcode_pos >= 0)) {
      ...
      if (intletter_index < 0) {
        // Did not!
        rr_throw((string) "I found an invalid character '" + chletter + "' in the code.");
      }
        ...
        if (intcode_pos > 0) {
          ...
          if (intprev_letter_index >= 0) {
            ...
            if (chletter == chprev_letter) {
              // Letter is repeated
              rr_throw((string) "A repeated letter '" + chletter + "' was found in the code.");
            }
            else { 
            ...
            rr_throw((string) "I found an invalid character '" + chprev_letter + "' in the code.");
          }          
        }
      }
      ...
      // Go to the previouHe-+s code letter.
      --intcode_pos;
    }
  }
}
...
string char_array_maths::extract_string(const string strAllowedChars, const char_repetition char_repeat, const int intmax_chars) {
  ...
  while ((!iszero()) && (intLetterNum <= intmax_chars)) {
    ...
    if ((char_repeat == char_repeat_not_allowed) && (lngAllowedIndex >= lngPrevAllowedIndex)) {
