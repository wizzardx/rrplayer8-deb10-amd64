...
// Types & constants used by run_find_script_class:
  ...
  // A definition of the commands accepte by run_find_script
  struct cmd_template {
...
public:
  // Initializer: Takes the string to be searched;
...
testing_func_throw;
      ...
      if ((strvar[0] == 'I' || strvar[0] == 'S') && 
        ...
        switch (strvar[0]) {
          ...
          default: {
            my_throw("Logic error! Unknown var type " + strvar[0]);
          }
        }
  ...
  void set_user_var(const string & strvar, string & strvalue) {
    ...
    while (!blncmd_found && cmd_templates[cmd_list_index].cmd != "")  {
      ...
      if ((cmd_templates[cmd_list_index].cmd == strcmd_name) && 
        ...
        while (!blnmismatch && 
          ...
          string strcmd_arg = command_parts;
          ...
          if (strtemplate_arg_type == "I") {
            ...
            try {
              ...
              strtemp_args += " " + fetch_int_value(strcmd_arg);              
            } catch(...) {
      ...
      ++cmd_list_index; // Go to the next command      
    }
...
testing_func_throw;
    ...
    if (strcmd == "search_forwards" && cmd_parts.count() == 2) {
      string strsearch_for = "";
      bool blnquote_error;
      strsearch_for = unquote_string(cmd_parts[1], '"');
    ...
    else if (strcmd == "search_backwards" && cmd_parts.count() == 2) {
    ...
    else if (strcmd == "inc_pos" && cmd_parts.count() == 2) {
    ...
    else if (strcmd == "dec_pos" && cmd_parts.count() == 2) {
      int intval = strtoi(cmd_parts[1]);
      int intnew_pos = intpos - intval;
      if (intnew_pos < 0 || intnew_pos > strbuffer.length() - 1) {
    ...
    else if (strcmd == "set_block_start" && cmd_parts.count() == 1) {
      if (intpos == -1) {
        my_throw("Cannot set block start! Current position not set.");
      }
      intblock_start = intpos;
    }
    else if (strcmd == "set_block_end" && cmd_parts.count() == 1) {
      if (intpos == -1) {
        my_throw("Cannot set block end! Current position not set.");
      }
      intblock_end = intpos;
    }
    else if (strcmd == "return_string" && cmd_parts.count() == 2) {
      string str = unquote_string(cmd_parts[1], '"');
      // End result of search was found!
      strfound_string = str;
      blnstring_found = true; // End the script loop
    }
    else if (strcmd == "assign_string" && cmd_parts.count() == 3) {
      string strvar = cmd_parts[1];
      string strvalue = cmd_parts[2];
      if (!is_string_var(strvar)) {
        my_throw(strvar + " is not a string variable!");
      }
      bool blnquote_error = false;
      if (is_quoted_string(strvalue, '"', blnquote_error)) {
        strvalue = unquote_string(strvalue, '"');
      }
      set_user_var(strvar, strvalue);
    }
    else if (strcmd == "assign_int" && cmd_parts.count() == 3) {
      string strvar = cmd_parts[1];
      string strvalue = cmd_parts[2];
    ...
    else if (strcmd == "increment_int" && cmd_parts.count() == 3) {
testing_func_throw;         
    }
    else if (strcmd == "append_string" && cmd_parts.count() == 3) {
      string strvar = cmd_parts[1];
      string strvalue = cmd_parts[2];
     
      // Unquote the value if necessary:
      bool blnquote_error = true;
      if (is_quoted_string(strvalue, '"', blnquote_error)) {
        strvalue = unquote_string(strvalue, '"');
      }
      
      // Find if it is a string var to be appended to:
      if (strvar[0] != 'S' || !isdigit(strvar[1])) {
        my_throw("Invalid variable for string appeding: '" + strvar + "'");
      }

      // Append the var.
      user_vars.S[strtoi(substr(strvar, 1))] += strvalue;          
    }
    else if (strcmd == "decrement_int" && cmd_parts.count() == 3) {
testing_func_throw;
    }
    else if (strcmd == "print_string" && cmd_parts.count() == 2) {
      string str = unquote_string(cmd_parts[1], '"');
      cout << "PRINT: " << str << endl;
    }
    else if (strcmd == "") {
...
testing_func_throw;              
  ...
  string fetch_int_value(const string & strvar) {
testing_func_throw;              
    string strret = "";
    bool blnerror = false; // Set to true if there is a problem
...
testing_func;    
    ...
    // Is this a string variable? eg "S5"    
      ...
      string strvalue = "";
      strvalue = get_user_var(strvar);
      if (!isint(strvalue)) {
        my_throw("Variable " + strvar + " does not contain an int!");
      }
      strret = itostr(strtoi(strvalue)); // Remove any leading 0's...
    }
...
void run_search_script(const string & strbuffer, const string & strsearch_script, 
                     string & strresult) {
