...
void mp3_tags::init(const string strBuffFilePath) {  // Initialize the mp3_tags object, provide a buffer filename  
  ...
  if (!FileExists("/usr/bin/mp3info")) {
    rr_throw("The mp3info tool is not installed!");
  }
  ...
  if (!tag_list_file) {
    rr_throw("Unable to open " + strTagBufferFile);
  }
  ...
  while (tag_list_file.getline(ch_FileLine, 2047)) {
    ...
    strLine = remove_ending_cr(strLine);
    ...
    if (strEND != "!!") {
      rr_throw("Bad output returned from mp3info!");
    }
    ...
    if (!file_tag_cached(strmp3_path)) {
      if (FileExists(strmp3_path)) {
        long lngFileSize = strtol(strmp3_size);
        if ((lngFileSize > 0) && (lngFileSize==FileSize(strmp3_path))) {
...
string mp3_tags::get_mp3_description(const string strFilePath) {
  ...
  // - Return "" if there is a problem, eg no tag in the MP3. The calling function should
  //   detect this and use the filename if the tag could not be retrieved.
  string strRetstr = "";
      ...
      if (FileExists("/usr/bin/mp3info")) {
        ...
        if (system(strCmd.c_str()) == 0) {
          ...
          if (!mp3info) {
            ...
            log_error("Could not open mp3info.txt");
          }
          else {
              ...
              if (!blnQuitFile) {
                switch (intLineNum) {
                  case 1: {
                    ...
                    else log_error("Invalid 1 line read from mp3info.txt: " + strLine);
                    break;
                  }
                  case 2: {
                    ...
                    else log_error("Invalid line 2 read from mp3info.txt: " + strLine);
                    break;
                  }
                  case 3: {
                    ...
                    else log_error("Invalid line 3 read from mp3info.txt: " + strLine);
                    break;
                  }
                  default: log_error("Too many lines read from mp3info.txt");
                }
              }
            }
            ...
            cache_file_tag(strFilePath, FileSize(strFilePath),strArtist, strAlbum, strTrackName, true);
          ...
          rr_throw("There was an error shelling " + strCmd);
        }
      }
      else {
        // mp3info not installed
        rr_throw("mp3info not installed!");
      }
    }
    else {
  ...
  } else log_warning(strFilePath + " not found."); //  Mp3 to fetch tag from does not exist.
                                                                            // Could be because of an XMMS freeze or because XMMS does not yet have a title ready?
  return strRetstr;
}
...
void mp3_tags::save_changes() {
  ...
  if (blnTagDetailsChanged) {
    // Log a message whenever the file is saved.
   log_message("Saving buffered mp3 tag details...");
    ...
    if (buffer_file) {
      ...
      while (item!=mp3_info.end()) {
        ...
        if (FileExists((*item).second.strMP3Path)) {
          blnSave = FileSize((*item).second.strMP3Path) == (*item).second.lngFileSize;
        }
        if (blnSave) {
    ...
    else rr_throw("Could not open " + strTagBufferFile + " for writing.");
  }
}
