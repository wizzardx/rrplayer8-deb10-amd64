...
string_splitter::string_splitter(const string & strsearch_in, const string & strdivider, const char chquote){
  ...
  if (strdivider == "") my_throw("String divider is empty!");

  // Setup str, used in other functions for error-logging:
  str = strsearch_in;
...
string string_splitter::next() {
  ...
  else my_throw("No more sub-strings left! String: \"" + str + "\"");
  return strret;
}

string string_splitter::operator[](const unsigned int n) const {
