...
datetime make_datetime(int year, int month, int day, int hour, int minute, int second) {
  ...
  if (dtmret == -1) {
    ...
    if (tm_time.tm_year   != tm_check.tm_year ||
        ...
        ) LOGIC_ERROR; // At least one of the fields is different. mktime really did have an error.
...
datetime parse_date_string(const string & strdate) {
  // Take a simple string of the layout dd/mm/yyyy or yyyy-mm-dd and convert it to a DateTime var
  if (strdate.length() != 10) my_throw("Date string must be 10 characters long! '" + strdate + "' has " + itostr(strdate.length()) + " characters.");
  ...
  if (DateStr[2] == '/') {
    ...
    if (!(isdigit(DateStr[0]) && isdigit(DateStr[1]) && DateStr[2] == '/' &&
      ...
      my_throw("Bad date string " + strdate);
    }
    intDay   = strtoi(substr(strdate, 0, 2));
    intMonth = strtoi(substr(strdate, 3, 2));
    intYear  = strtoi(substr(strdate, 6, 4));
  }
  else if(DateStr[4] == '-') {
    // Check for "yyyy-mm-dd"
    if (!(isdigit(DateStr[0]) && isdigit(DateStr[1]) && isdigit(DateStr[2]) && isdigit(DateStr[3]) && DateStr[4] == '-' &&
        isdigit(DateStr[5]) && isdigit(DateStr[6]) && DateStr[7] == '-' &&
        isdigit(DateStr[8]) && isdigit(DateStr[9]))) {
      my_throw("Bad date string " + strdate);
    }
    intYear  = strtoi(substr(strdate, 0, 4));
    intMonth = strtoi(substr(strdate, 5, 2));
    intDay   = strtoi(substr(strdate, 8, 2));
  }
  else my_throw("Unrecognised date format in this string: " + strdate);
  ...
  string strTestDate = "";
  if (DateStr[2] == '/') {
    strTestDate = format_datetime(dtmDate, "%d/%m/%Y");
  }
  else {
    strTestDate = format_datetime(dtmDate, "%Y-%m-%d");
  }
...
datetime parse_datetime_string(const string & strdatetime) {
  ...
  // [date] is in the format "dd/mm/yyyy" or "yyyy-mm-dd" and time is in the format "hh:nn:ss ?m".
  // However, with [time], the ":ss" and "?m" parts are optional (ps: ?m means AM or PM)
  ...
  int intPosSlash = strwork.find("/");
  int intPosMinus = strwork.find("-");

  if (intPosSlash < 0 && intPosMinus < 0) {
    // There were no "/" or "-" characters found. Ie the string probably is just a time
    strTime = strwork;
  }
  else {
    // A "/" or "-" character was found. ie there is probably a date in the string. Check where it was found.
    if (intPosSlash == 2 || intPosMinus == 4) {
      // The 3rd character was a "/" or the 5th character was "-".
      // This means that the date would be in the beginning.
      ...
      // "/" or "-" was found at a different character position. This means that the date is probably
      // found after the time
      if (intPosSlash >= 0) {
        // - Extract the section before the date
        strTime = substr(strwork, 0, intPosSlash - 3);
        // - Now extract the date.
        strDate = trim(substr(strwork, intPosSlash - 3));
      }
      else if (intPosMinus >= 0) {
        // - Extract the section before the date
        strTime = substr(strwork, 0, intPosMinus - 5);
        // - Now extract the date.
        strDate = trim(substr(strwork, intPosMinus - 5));
      }
    }
  }
  ...
  if ((strDate != "") && (strTime != "")) {
    // Using datetime_to_tm and mktime instead of direct mathematics, to avoid
    // problems caused by daylight savings!

    // Convert dtmdate and dtmtime to a tm struct:
    tm tm_date = datetime_to_tm(dtmDate);
    tm tm_time = datetime_to_tm(dtmTime);

    // Build the result:
    tm tm_ret = tm_date; //
    tm_ret.tm_hour = tm_time.tm_hour;
    tm_ret.tm_min  = tm_time.tm_min;
    tm_ret.tm_sec  = tm_time.tm_sec;

    dtmRet = mktime(&tm_ret);
  }
...
long date_diff(const string & interval, const datetime date1, const datetime date2) {
  ...
  if (interval == "yyyy") {
  ...
  #ifdef __linux__
    return (date2-get_tm_gmtoff(date2))/(24*60*60) - (date1-get_tm_gmtoff(date1))/(24*60*60);
  #else
    undefined_throw;
  #endif
  }
    ...
    // Hour
  #ifdef __linux__
    return  (date2-get_tm_gmtoff(date2))/(60*60) - (date1-get_tm_gmtoff(date1))/(60*60);
  #else
    undefined_throw;
  #endif
  }
    ...
    // Minute
  #ifdef __linux__
    return (date2-get_tm_gmtoff(date2))/60 - (date1-get_tm_gmtoff(date1))/60;
  #else
    undefined_throw;
  #endif
  }
