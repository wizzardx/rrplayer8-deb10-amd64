...
windows_mount::windows_mount(const string & strWindowsPath, const string & strlogin_user, const string & strencrypted_login_password, const string & strmnt_descr_, const unsigned long long lngmin_free_space) {
  ...
  do_mount(strWindowsPath, strlogin_user, strencrypted_login_password, strmnt_descr_, lngmin_free_space);
}
...
void windows_mount::do_mount(const string & strWindowsPath, const string & strlogin_user, const string & strencrypted_login_password, const string & strmnt_descr_, const unsigned long long lngmin_free_space) {
  ...
  strmount_dir = "";
  strfull_path = "";
  blncreated_mount = false; // Set to true if this object instance created (or attempted to create) the mount.
  ...
  if (strmnt_descr_ == "") {
    ...
    rr_throw("mount description argument is empty (" + strWindowsPath + ")");
  }

  strmnt_descr = strmnt_descr_;  
  
  // Windows path
  if (strWindowsPath == "") {
    // Windows path not set!
    rr_throw("Windows path argument is empty (" + strmnt_descr + ")");
  }

  if (strlogin_user == "") {
    // Username for samba mount not set!
    rr_throw("samba user argument is empty (" + strmnt_descr + ")");
  }

  // login_password
  if (strencrypted_login_password == "") {
    // Windows path not set!
    rr_throw("samba password argument is empty (" + strmnt_descr +")");
  }

  // Is smbmount installed?
  if (!FileExists("/usr/bin/smbmount")) {
    // smbmount is not installed!
    rr_throw("smbmount not found, please install Debian package \"smbfs\"");
  }

  // Copy the windows path so that "remote_path()" can retrieve it.
  strwindows_path = strWindowsPath;
  ...
  // - First, replace "\\" with "/"
  string strTemp(strWindowsPath);
  strTemp = replace(strTemp, "\\", "/");
  // Are the first 2 characters "//" ?
  if (substr(strTemp, 0, 2) != "//") {
    rr_throw("Windows path \"" + strWindowsPath + "\" (" + strmnt_descr + ") must start with \"\\\\\"");
  }

  // Is there another "/" in the path after the initial "//" ?
  int intslash_3_pos = strTemp.find("/", 2);
  int intslash_4_pos = strTemp.find("/", intslash_3_pos + 1);

  if (intslash_3_pos < 0) {
    rr_throw("Windows path \"" + strWindowsPath + "\" (" + strmnt_descr + ") must have another \"\\\" character after the initial \"\\\\\"");
  }

  //There must be more characters after the 3rd "\"
  if ((signed) strTemp.length() <= (intslash_3_pos + 1)) {
    rr_throw("Windows path \"" + strWindowsPath + "\" (" + strmnt_descr + ") must have more characters after the 3rd \"\\\" character");
  }
  ...
  if (intslash_4_pos < 0) {
    ...
    strservice_name = strTemp;
    strservice_subdir = "";
  }
  else {
    // We have a 4th slash, the service name and subdirectory are
    // separated by this character.
    strservice_name = substr(strTemp, 0, intslash_4_pos);
    strservice_subdir = substr(strTemp, intslash_4_pos + 1);
  }

  // Check that we extracted a service name:
  if (strservice_name == "") {
    rr_throw("Error extracting service name from windows path \"" + strWindowsPath + "\" (" + strmnt_descr+ ")");
  }
  ...
  if (strservice_subdir != "") {
    strservice_subdir = ensure_char_at_end(strservice_subdir, '/');
  }
  ...
  strmount_dir=strmount_pid + StringToLower(replace(substr(strservice_name, 2), "/", "_") + "/");
  ...
  if (DirExists(strmount_dir)) {
    ...
    if (strmounted_service == "") {
      ...
      if (strmounted_service == strservice_name) {
        // Yes: The share was already created. We will attempt to reuse it.
        blnreuse_mount = true;
      }
      else {   
        // No: An unknown service is already mounted there!
        rr_throw("An unknown service \"" + strmounted_service + "\" is already mounted at " + strmount_dir);
      }
    }                                
  }
  ...
  if (!blnreuse_mount) {
    ...
    string strdecrypted_password=strencrypted_login_password;
    string_encryption StringEnc(strdecrypted_password);

    StringEnc.decrypt(get_rr_encrypt_key(), 2); // Throws an exception on failure.
    ...
    if (intsmbmount_ret != 0 || strsmbmount_out != "") {
      rr_throw("Could not mount \"" + strservice_name + "\" (" + strmnt_descr + "). " + strsmbmount_out);
    }
  }

  // The share is now mounted. Does the subdirectory the user is interested in, exist?
  if (!DirExists(strmount_dir + strservice_subdir)) {
    rr_throw("Directory does not exist: " + strWindowsPath + "(" + strmnt_descr + ")");
  ...
  if (lngavailable < lngmin_free_space) {
    ...
    rr_throw("Free space is running low! " + strWindowsPath + " has only " + ltostr(lngMB) + " MB remaining!");
  }
}
...
void windows_mount::do_unmount() {
  ...
  if (blncreated_mount) {
    ...
    if (intsmbmount_ret != 0 || strsmbmount_out != "") {
      // Unmount failed!
      rr_throw("Could not unmount directory " + strmount_dir + " (" + strmnt_descr + "). " + strsmbmount_out);
    } 
    else {
...
void windows_mount::tidy_old_mounts() {
  ...
  while (strpid != "") {
    ...
    if (!blnpid_found) {
      ...
      string strmnt = pid_subdirs.item();
      while (strmnt != "") {
        ...
        // Fetch the next mount subdirectory:
        strmnt = pid_subdirs.item();
      } // END: while (strmnt != "")
...
string windows_mount::get_first_mounted_samba_service(const string & strpath) {
  ...
  if (intret != 0) {
    ...
    rr_throw("Unexpected return code!");
  }
