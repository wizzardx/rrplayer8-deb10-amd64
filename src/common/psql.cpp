...
// 1) Install libpqxx-dev
// 2) Include the following CPPFLAGS: -I /usr/include/postgresql/
// 3) Link against the following libraries: /usr/lib/libpqxx.so
...
void pg_connection::check() {
  ...
  if (pconn == NULL) {
    my_throw("Connection is not meant to be open, check() should not be called. Call open() first.");
  }
  establish_connection();
}
...
pg_result pg_connection::exec(const string & strsql) { // An exception is thrown if there is a SQL execution error
  ...
  if (ptransaction == NULL) my_throw("Transaction object not initialized");
...
void pg_result::movenext() { // An exception is thrown if an attempt is made to move past the EOF.
  if (!eof()) {
    ...
    my_throw("Attempt to move past the EOF");
  }
}
